...
if not os.path.exists('model_detection_sft_is_af'):
    os.mkdir('model_detection_sft_is_af')
log_path = os.path.join('model_detection_sft_is_af', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_af/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_af/sdnet_%d.pth' % (epoch + 1)))
