...
from model_detection_sft import VanishingNet
from datasets import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="5,0,3,4,6,1,2,7"
if not os.path.exists('model_detection_sft'):
    os.mkdir('model_detection_sft')
log_path = os.path.join('model_detection_sft', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_detection_sft/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft/sdnet_%d.pth' % (epoch + 1)))
