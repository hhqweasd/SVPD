...
os.environ["CUDA_VISIBLE_DEVICES"]="0,3,2,7,1,5,6,4"
...
for ee in range(100,69,-1):
    # ee=92
    generator_1='model_svp_is_abl_wosft_woi_run3/vpnet_%s.pth'%(ee)
    generator_2='model_svp_is_abl_wosft_woi_run3/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_svp_is_abl_wosft_woi_run3/maskis'):
        os.makedirs('model_svp_is_abl_wosft_woi_run3/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_is_abl_wosft_woi_run3/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_svp_is_abl_wosft_woi_run3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_abl_wosft_woi_run3/dist_%s.mat'%(ee)
