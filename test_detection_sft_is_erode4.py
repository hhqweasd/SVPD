...
os.environ["CUDA_VISIBLE_DEVICES"]="3,2,7,6,4,1,0,5"
...
for ee in range(100,90,-1):
    generator_1='model_detection_sft_is_erode4_run2/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_erode4_run2/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_erode4_run2/maskis'):
        os.makedirs('model_detection_sft_is_erode4_run2/maskis')
    ...
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection_sft_is_erode4_run2/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_erode4_run2/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_erode4_run2/dist_%s.mat'%(ee)
