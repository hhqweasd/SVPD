...
from datasets_vp4 import ImageDataset
import numpy as np
...
os.environ["CUDA_VISIBLE_DEVICES"]="4,2,3,0,7,5,6,1"
...
if not os.path.exists('model_vp_conic'):
    os.mkdir('model_vp_conic')
opt.log_path = os.path.join('model_vp_conic', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    torch.save(vpnet.state_dict(), 'model_vp_conic/vpnet.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(vpnet.state_dict(), ('model_vp_conic/vpnet_%d.pth' % (epoch + 1)))
