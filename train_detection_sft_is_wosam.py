...
from model_vp_conic_256 import VanishingNet
# from datasets import ImageDataset
from datasetis import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="3,4,0"
if not os.path.exists('model_detection_sft_is_wosam'):
    os.mkdir('model_detection_sft_is_wosam')
log_path = os.path.join('model_detection_sft_is_wosam', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask_IS,realco) in enumerate(dataloader):
        vpnet.train()
        sdnet.train()
        ...
        result=vpnet(torch.cat((real_nsr,sdmask.sigmoid()),1),realco) 
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_wosam/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_wosam/sdnet_%d.pth' % (epoch + 1)))
