...
os.environ["CUDA_VISIBLE_DEVICES"]="3,2,7,6,4,0,5,1"
...
for ee in range(100,90,-1):
    ee=95
    generator_1='model_detection_sft_is_wot_wogt/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_wot_wogt/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_wot_wogt_data3'):
        os.makedirs('model_detection_sft_is_wot_wogt_data3')
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        save_result='./model_detection_sft_is_wot_wogt_data3/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_wot_wogt_data3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_wot_wogt_data3/dist_%s.mat'%(ee)
