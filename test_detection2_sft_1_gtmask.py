...
for ee in range(100,90,-1):
    generator_1='model_detection2_sft_1_run3/vpnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco,mask_IS) in enumerate(dataloader):
        ...
        save_result='./model_detection2_sft_1_run3/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection2_sft_1_run3/err_gtmask_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection2_sft_1_run3/dist_gtmask_%s.mat'%(ee)
