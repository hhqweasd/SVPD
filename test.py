...
# dataroot = '/home/liuzhihao/dataset/SVPI/vpdataset3'
...
for ee in range(77,69,-1):
    ...
    if not os.path.exists('model_svp_is_run2/mask'):
        os.makedirs('model_svp_is_run2/mask')
    ...
    for image_index, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_is_run2/mask/%s'% (str(image_index) + '.png'))
    ...
    # err = np.array(err)
    # nnn='model_svp_is_run2/err_%s.mat'%(ee)
    # scipyio.savemat(nnn, {'name': err})
    # errdist = np.array(errdist)
    # nnn='model_svp_is_run2/dist_%s.mat'%(ee)
    # scipyio.savemat(nnn, {'name': errdist})
    
    err = np.array(err)
    nnn='model_svp_is_run2/err.mat'
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_run2/dist.mat'
    scipyio.savemat(nnn, {'name': errdist})
