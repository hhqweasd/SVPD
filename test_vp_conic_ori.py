...
os.environ["CUDA_VISIBLE_DEVICES"]="1,4,3,2,7,5,6,0"
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_ori/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_ori/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_ori/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_ori/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_ori/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_ori/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_ori/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_ori/dist_%s.mat'%(ee)
