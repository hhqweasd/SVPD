...
from model_detection_sft_is_dia3 import VanishingNet
# from datasets import ImageDataset
from datasetis import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="6,5,1,2,3,4,0,7"
if not os.path.exists('model_detection_sft_is_dia3'):
    os.mkdir('model_detection_sft_is_dia3')
log_path = os.path.join('model_detection_sft_is_dia3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_dia3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_dia3/sdnet_%d.pth' % (epoch + 1)))
