...
os.environ["CUDA_VISIBLE_DEVICES"]="7,5,0,1,2,3,6,4"
if not os.path.exists('model_detection_sft_is_data3_sdvp'):
    os.mkdir('model_detection_sft_is_data3_sdvp')
log_path = os.path.join('model_detection_sft_is_data3_sdvp', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        sdmask,s1,s2,s3,s4,s5,s6,s7,s8=sdnet(real_nsr,realco)             
        ...
        if (i+1) % opt.iter_loss == 0:
            log = 'Epoch: %d,[loss_sd %.5f]'%(epoch,loss_sd,)
            print(log)
            open(log_path, 'a').write(log + '\n')

    if epoch>84:
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_data3_sdvp/sdnet_%d.pth' % (epoch + 1)))
