...
for ee in range(100,90,-1):
    ee=96
    generator_1='model_detection_sft_is_wogt/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_wogt/sdnet_%s.pth'%(ee)
    vpnet.load_state_dict(torch.load(generator_1))
    vpnet.eval()
    sdnet.load_state_dict(torch.load(generator_2))
    sdnet.eval()

    if not os.path.exists('model_detection_sft_is_wogt_data3'):
        os.makedirs('model_detection_sft_is_wogt_data3')
    err = []
    errdist = []
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        save_result='./model_detection_sft_is_wogt_data3/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_wogt_data3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_wogt_data3/dist_%s.mat'%(ee)
