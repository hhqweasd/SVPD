...
os.environ["CUDA_VISIBLE_DEVICES"]="3,4,1,2,0"
if not os.path.exists('model_detection_sft_is_abl_bf'):
    os.mkdir('model_detection_sft_is_abl_bf')
log_path = os.path.join('model_detection_sft_is_abl_bf', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_abl_bf/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_abl_bf/sdnet_%d.pth' % (epoch + 1)))
