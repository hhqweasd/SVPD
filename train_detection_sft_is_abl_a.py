...
if not os.path.exists('model_detection_sft_is_a_data3'):
    os.mkdir('model_detection_sft_is_a_data3')
log_path = os.path.join('model_detection_sft_is_a_data3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
sdnet=BDRAR()
sdnet.load_state_dict(torch.load('vps_3001_data3.pth'))
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_a_data3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_a_data3/sdnet_%d.pth' % (epoch + 1)))
