import glob
import random
import os

from torch.utils.data import Dataset
from skimage import io, color
from skimage.transform import rescale, resize, downscale_local_mean
import random
import numpy as np
import torch
import string
import numpy.linalg as LA

class ImageDataset_IS(Dataset):
    def __init__(self, root,mode='train'):
        if mode=='train':
            self.files_A = sorted(glob.glob(os.path.join(root, '%s/train_A_256' % mode) + '/*.*'))
            self.files_B = sorted(glob.glob(os.path.join(root, '%s/train_B_256_s' % mode) + '/*.*'))
            self.files_C = sorted(glob.glob(os.path.join(root, '%s/train_C' % mode) + '/*.*'))
            self.files_D = sorted(glob.glob(os.path.join(root, '%s/train_B_256' % mode) + '/*.*'))
        
    def __getitem__(self, index):
        k=random.randint(0,100)
        kk=random.randint(0,100)
        item_A=io.imread(self.files_A[index % len(self.files_A)])
        ori_x=3024.0
        ori_y=4032.0
        if k>50:
            item_A=np.fliplr(item_A)   
        if kk>50:            
            item_A=np.flipud(item_A) 
        item_B=color.rgb2gray(io.imread(self.files_B[index % len(self.files_B)]))
        if k>50:
            item_B=np.fliplr(item_B)
        if kk>50:            
            item_B=np.flipud(item_B) 
        item_D=color.rgb2gray(io.imread(self.files_D[index % len(self.files_D)]))
        if k>50:
            item_D=np.fliplr(item_D)
        if kk>50:            
            item_D=np.flipud(item_D) 
        item_A=np.asarray(item_A)/255.0
        item_A=torch.from_numpy(item_A.copy()).float()
        item_A=item_A.view(256,256,3)
        item_A=item_A.transpose(0, 1).transpose(0, 2).contiguous()
        
        item_B=np.asarray(item_B)
        item_B=torch.from_numpy(item_B.copy()).float()
        item_B=item_B.view(256,256,1)
        item_B=item_B.transpose(0, 1).transpose(0, 2).contiguous()
        item_D=np.asarray(item_D)
        item_D=torch.from_numpy(item_D.copy()).float()
        item_D=item_D.view(256,256,1)
        item_D=item_D.transpose(0, 1).transpose(0, 2).contiguous()
 
        x,y=[],[]
        f = open(self.files_C[index % len(self.files_C)],"r")
        for eachline in f:
            tmp = eachline.split()
            x=(np.asarray(tmp[1],dtype=float))
            x=x*256.0/ori_x
            y=(np.asarray(tmp[0],dtype=float))
            y=y*256.0/ori_y
            if k>50:
                y=256.0-y
            if kk>50:            
                x=256.0-x
        f.close()
        item_C=np.array([[x/128-1.0,1.0-y/128,1.0]])# 为什么y取负（宽），因为python的y轴从上到下
        item_C[0] /= LA.norm(item_C[0])
        item_C=torch.from_numpy(item_C.copy()).float()
        return item_A,item_B,item_C,item_D
        # 图像是否需要归一化？
        # 就不裁剪了，怕把影子减掉
        
        
    def __len__(self):
        return max(len(self.files_A), len(self.files_B), len(self.files_C))


class ImageDataset(Dataset):
    def __init__(self, root,mode='train'):
        if mode=='train':
            self.files_A = sorted(glob.glob(os.path.join(root, '%s/train_A_256' % mode) + '/*.*'))
            self.files_B = sorted(glob.glob(os.path.join(root, '%s/train_B_256_s' % mode) + '/*.*'))
            self.files_C = sorted(glob.glob(os.path.join(root, '%s/train_C' % mode) + '/*.*'))
        
    def __getitem__(self, index):
        k=random.randint(0,100)
        kk=random.randint(0,100)
        item_A=io.imread(self.files_A[index % len(self.files_A)])
        ori_x=3024.0
        ori_y=4032.0
        if k>50:
            item_A=np.fliplr(item_A)   
        if kk>50:            
            item_A=np.flipud(item_A) 
        item_B=color.rgb2gray(io.imread(self.files_B[index % len(self.files_B)]))
        if k>50:
            item_B=np.fliplr(item_B)
        if kk>50:            
            item_B=np.flipud(item_B) 
        item_A=np.asarray(item_A)/255.0
        item_A=torch.from_numpy(item_A.copy()).float()
        item_A=item_A.view(256,256,3)
        item_A=item_A.transpose(0, 1).transpose(0, 2).contiguous()
        
        item_B=np.asarray(item_B)
        item_B=torch.from_numpy(item_B.copy()).float()
        item_B=item_B.view(256,256,1)
        item_B=item_B.transpose(0, 1).transpose(0, 2).contiguous()
 
        x,y=[],[]
        f = open(self.files_C[index % len(self.files_C)],"r")
        for eachline in f:
            tmp = eachline.split()
            x=(np.asarray(tmp[1],dtype=float))
            x=x*256.0/ori_x
            y=(np.asarray(tmp[0],dtype=float))
            y=y*256.0/ori_y
            if k>50:
                y=256.0-y
            if kk>50:            
                x=256.0-x
        f.close()
        item_C=np.array([[x/128-1.0,1.0-y/128,1.0]])# 为什么y取负（宽），因为python的y轴从上到下
        item_C[0] /= LA.norm(item_C[0])
        item_C=torch.from_numpy(item_C.copy()).float()
        return item_A,item_B,item_C
        # 图像是否需要归一化？
        # 就不裁剪了，怕把影子减掉
        
        
    def __len__(self):
        return max(len(self.files_A), len(self.files_B), len(self.files_C))

