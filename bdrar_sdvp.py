...
from resnext.resnext101_regular4 import ResNeXt101
...
class BDRAR(nn.Module):
    ...
    def forward(self, x,vps):
        ...
        vpf=self.fc2(vpf)
        vpf=self.fc3(vpf)
        vpf=vpf.view(vpf.size(0),1,64,64)
        vpf = F.interpolate(vpf, [int(x.size(2)),int(x.size(3))], mode='bilinear', align_corners=True)
        
        layer0 = self.layer0(torch.cat((x,vpf),1))
