...
class Bottleneck2D(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class Bottleneck2D1(nn.Module):
    expansion = 2

    def __init__(self, inplanes, planes, stride=1, resample=None):
        super(Bottleneck2D1, self).__init__()

        self.bn1 = nn.BatchNorm2d(inplanes)
        self.conv1 = nn.Conv2d(inplanes+1, planes, kernel_size=1)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1)
        self.bn3 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes * Bottleneck2D.expansion, kernel_size=1)
    ...
    def forward(self, x):
        ...
        out = self.bn1(x[:,:-1])
        out = self.relu(out)
        out = self.conv1(torch.cat((out,mask),1))
...
class HourglassNet(nn.Module):
    ...
    def forward(self, x,mask):
        ...
        x = x + x * mask.repeat(1, x.size(1), 1, 1)
        x = self.layer1(x)
        x = self.maxpool(x)
        mask = F.interpolate(mask, scale_factor=0.5)
        x = x + x * mask.repeat(1, x.size(1), 1, 1)
