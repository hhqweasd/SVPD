...
from model_vp2 import Discriminator
...
if not os.path.exists('model_vp2'):
    os.mkdir('model_vp2')
opt.log_path = os.path.join('model_vp2', str(datetime.datetime.now()) + '.txt')
...
input_B = Tensor(opt.batchSize, 1, 500, 500)
input_C = Tensor(1,2)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        # Set model input
        real_nsr = Variable(input_A.copy_(batch['A']))#
        real_mask = Variable(input_B.copy_(batch['B']))#
        realco = Variable(input_C.copy_(batch['C']))#
        ...
        same_B = netG_A2B(real_nsr,real_mask)
    ...
    torch.save(netG_A2B.state_dict(), 'model_vp2/netG_A2B.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(netG_A2B.state_dict(), ('model_vp2/netG_A2B_%d.pth' % (epoch + 1)))
