...
from model_detection_sft_is_conv import VanishingNet
...
os.environ["CUDA_VISIBLE_DEVICES"]="0,1,7,6,5,2,3,4"
if not os.path.exists('model_conv_run1'):
    os.mkdir('model_conv_run1')
log_path = os.path.join('model_conv_run1', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_conv_run1/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_conv_run1/sdnet_%d.pth' % (epoch + 1)))
