...
for ee in range(77,69,-1):
    # ee=92
    generator_1='model_svp_is_run2/vpnet_%s.pth'%(ee)
    generator_2='model_svp_is_run2/sdnet_%s.pth'%(ee)
    ...
    for image_index, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_is_run2/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_svp_is_run2/err_fixed_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_run2/dist_fixed_%s.mat'%(ee)
    ...
    # if not os.path.exists('model_svp_is_run2/maskis_data3'):
        # os.makedirs('model_svp_is_run2/maskis_data3')
        
    # # for i, (real_nsr,mask,realco) in enumerate(dataloader):
    # for image_index2, (real_nsr,realco) in enumerate(dataloader):
        # real_nsr=real_nsr.cuda()
        # sdmask=sdnet(real_nsr)   
        ...
        # # fake_B448=resize(fake_B448,(256,256,1))
        # # outputimage=fake_B448
        # # save_result=join_paths('./model_svp_is_run2/maskis_data3/%s'% (str(i) + '.png'))
        # # io.imsave(save_result,outputimage)
    ...
    # err = np.array(err)
    # nnn='model_svp_is_run2/err_data3_%s.mat'%(ee)
    # scipyio.savemat(nnn, {'name': err})
    # errdist = np.array(errdist)
    # nnn='model_svp_is_run2/dist_data3_%s.mat'%(ee)
    # scipyio.savemat(nnn, {'name': errdist})
