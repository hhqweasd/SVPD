...
from dataset_vpnet import ImageDataset

import warnings
warnings.filterwarnings("ignore")
os.environ["CUDA_VISIBLE_DEVICES"]="0,5,7,4,3,6,2,1"
if not os.path.exists('model_vpnet_data3_run2'):
    os.mkdir('model_vpnet_data3_run2')
log_path = os.path.join('model_vpnet_data3_run2', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>(opt.n_epochs-11):
        torch.save(vpnet.state_dict(), ('model_vpnet_data3_run2/vpnet_%d.pth' % (epoch + 1)))
