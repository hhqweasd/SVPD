...
if not os.path.exists('model_vp_conic_convanchors_ca_mask'):
    os.mkdir('model_vp_conic_convanchors_ca_mask')
opt.log_path = os.path.join('model_vp_conic_convanchors_ca_mask', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    # torch.save(vpnet.state_dict(), 'model_vp_conic_convanchors_ca_mask/vpnet.pth')

    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_convanchors_ca_mask/vpnet_%d.pth' % (epoch + 1)))
    ...
    if epoch>90:
        with torch.no_grad(): 
            ...
            if mean_loss < best_mean_loss:
                ...
                torch.save(vpnet.state_dict(), ('model_vp_conic_convanchors_ca_mask/vpnet_best.pth'))
