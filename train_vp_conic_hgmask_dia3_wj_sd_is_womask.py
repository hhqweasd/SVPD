...
if not os.path.exists('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001_womask'):
    os.mkdir('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001_womask')
opt.log_path = os.path.join('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001_womask', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        result=vpnet(real_nsr,realco,sdmask.detach()) 
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001_womask/vpnet_%d.pth' % (epoch + 1)))
