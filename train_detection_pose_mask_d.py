...
from model_detection_pose_mask_d import VanishingNet
from datasets import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="0,1,2,3,4,5,6,7"
if not os.path.exists('model_detection_pose_mask_d'):
    os.mkdir('model_detection_pose_mask_d')
log_path = os.path.join('model_detection_pose_mask_d', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_detection_pose_mask_d/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_pose_mask_d/sdnet_%d.pth' % (epoch + 1)))
