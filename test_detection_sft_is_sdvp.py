...
for ee in range(100,99,-1):
    generator_2='model_detection_sft_is_data3_sdvp/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_data3_sdvp/maskis'):
        os.makedirs('model_detection_sft_is_data3_sdvp/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection_sft_is_data3_sdvp/maskis/%s'% (str(i) + '.png'))
