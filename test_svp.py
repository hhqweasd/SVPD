...
os.environ["CUDA_VISIBLE_DEVICES"]="2,5,1,0,4,3,7,6"
...
for ee in range(100,74,-1):
    # ee=92
    generator_1='model_svp_run4/vpnet_%s.pth'%(ee)
    generator_2='model_svp_run4/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_svp_run4/maskis'):
        os.makedirs('model_svp_run4/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_run4/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_svp_run4/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_run4/dist_%s.mat'%(ee)
