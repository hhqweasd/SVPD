...
from datasets_vp_full_s_256_test import ImageDataset
...
os.environ["CUDA_VISIBLE_DEVICES"]="0,1,2,5,3,4,7,6"
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_hgmask_dia3_sd_s/vpnet_%s.pth'%(ee)
    generator_2='model_vp_conic_hgmask_dia3_sd_s/sdnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco,name) in enumerate(dataloader):
        ...
        save_result='./model_vp_conic_hgmask_dia3_sd_s/mask.png'
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_hgmask_dia3_sd_s/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_hgmask_dia3_sd_s/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_hgmask_dia3_sd_s/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_hgmask_dia3_sd_s/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_hgmask_dia3_sd_s/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_hgmask_dia3_sd_s/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_hgmask_dia3_sd_s/dist_%s.mat'%(ee)
