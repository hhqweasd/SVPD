...
from model_vp_conic_hgmask_dia3 import VanishingNet
from datasets_vp_full_is import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="2,6,7,5,3,0,4,1"
...
if not os.path.exists('model_vp_conic_hgmask_dia3_wjcca_is'):
    os.mkdir('model_vp_conic_hgmask_dia3_wjcca_is')
opt.log_path = os.path.join('model_vp_conic_hgmask_dia3_wjcca_is', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_hgmask_dia3_wjcca_is/vpnet_%d.pth' % (epoch + 1)))
