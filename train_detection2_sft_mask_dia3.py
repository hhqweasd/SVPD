...
from model_detection2_sft_dia3 import VanishingNet
from datasets import ImageDataset_IS

os.environ["CUDA_VISIBLE_DEVICES"]="0,1,2,3,4,5,6,7"
if not os.path.exists('model_detection2_sft_dia3'):
    os.mkdir('model_detection2_sft_dia3')
log_path = os.path.join('model_detection2_sft_dia3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_detection2_sft_dia3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection2_sft_dia3/sdnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet_IS.state_dict(), ('model_detection2_sft_dia3/sdnet_IS_%d.pth' % (epoch + 1)))
