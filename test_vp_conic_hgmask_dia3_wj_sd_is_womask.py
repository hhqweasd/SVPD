...
from hourglass_pose_mask_sd_256 import hg
from model_vp_conic_hgmask_dia3_256_test import VanishingNet
import numpy.linalg as LA
import math
from datasets_vp_full_is_256_test import ImageDataset
...
os.environ["CUDA_VISIBLE_DEVICES"]="0,2,5,3,4,7,6,1"
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/vpnet_%s.pth'%(ee)
    generator_2='model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/sdnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco,name) in enumerate(dataloader):
        ...
        save_result='./model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/mask.png'
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_hgmask_dia3_wj_sd_is_vp3001/dist_%s.mat'%(ee)
