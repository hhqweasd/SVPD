...
from hourglass_pose_mask_wj2_newsft2  import hg
...
os.environ["CUDA_VISIBLE_DEVICES"]="2,3,6,7,4,5,0,1"
if not os.path.exists('model_new_sft2_lr_run6'):
    os.mkdir('model_new_sft2_lr_run6')
log_path = os.path.join('model_new_sft2_lr_run6', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>74:
        torch.save(vpnet.state_dict(), ('model_new_sft2_lr_run6/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_new_sft2_lr_run6/sdnet_%d.pth' % (epoch + 1)))
