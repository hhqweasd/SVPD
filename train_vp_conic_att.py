...
from model_vp_conic_att import VanishingNet
...
if not os.path.exists('model_vp_conic_att'):
    os.mkdir('model_vp_conic_att')
opt.log_path = os.path.join('model_vp_conic_att', str(datetime.datetime.now()) + '.txt')
...
vpnet=torch.nn.DataParallel(vpnet,device_ids=[0,1,2])
...
optimizer_G = torch.optim.Adam(vpnet.parameters(),lr=opt.lr*3,weight_decay=0.0006,amsgrad=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    torch.save(vpnet.state_dict(), 'model_vp_conic_att/vpnet.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(vpnet.state_dict(), ('model_vp_conic_att/vpnet_%d.pth' % (epoch + 1)))
    ...
    if epoch>80:
        with torch.no_grad(): 
            ...
            if mean_loss < best_mean_loss:
                ...
                torch.save(vpnet.state_dict(), ('model_vp_conic_att/vpnet_best.pth'))
