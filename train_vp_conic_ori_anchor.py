...
from model_vp_conic_anchor import VanishingNet
...
if not os.path.exists('model_vp_conic_ori_anchor'):
    os.mkdir('model_vp_conic_ori_anchor')
opt.log_path = os.path.join('model_vp_conic_ori_anchor', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    torch.save(vpnet.state_dict(), 'model_vp_conic_ori_anchor/vpnet.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(vpnet.state_dict(), ('model_vp_conic_ori_anchor/vpnet_%d.pth' % (epoch + 1)))
    ...
    with torch.no_grad(): 
        ...
        if mean_loss < best_mean_loss:
            ...
            torch.save(vpnet.state_dict(), ('model_vp_conic_ori_anchor/vpnet_best.pth'))
