...
from model_vp_conic_hgmask_test import VanishingNet
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_hgmask_wj2_is/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_hgmask_wj2_is/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_hgmask_wj2_is/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_hgmask_wj2_is/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_hgmask_wj2_is/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_hgmask_wj2_is/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_hgmask_wj2_is/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_hgmask_wj2_is/dist_%s.mat'%(ee)
