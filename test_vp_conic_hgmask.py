...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_hgmask_s/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    nnn='model_vp_conic_hgmask_s/err_%s.mat'%(ee)
    #np.savez('./model_vp_conic_hgmask_s/angle_98.npz', err=err)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    #np.savez('./model_vp_conic_hgmask_s/dist_98.npz', errdist=errdist)
    nnn='model_vp_conic_hgmask_s/dist_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': errdist})
    #scipyio.savemat('model_vp_conic_hgmask_s/imgname_98.mat', {'name': imgname})
