...
from hourglass_pose_withmask import hg
...
os.environ["CUDA_VISIBLE_DEVICES"]="5,2,7,6,4,3,0,1"
...
for ee in range(100,90,-1):
    generator_1='model_detection_sft_is_wosam/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_wosam/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_wosam/maskis'):
        os.makedirs('model_detection_sft_is_wosam/maskis')
    ...
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection_sft_is_wosam/maskis/%s'% (str(i) + '.png'))
        ...
        with torch.no_grad():
            score = vpnet(torch.cat((real_nsr,sdmask.sigmoid()),1),vpts)[:, -1].cpu().numpy()
        ...
        for res in range(1, len(multires)):
            ...
            vps = np.vstack(vpts)
            with torch.no_grad():
                score = vpnet(torch.cat((real_nsr,sdmask.sigmoid()),1),vps)[:, -res - 1].cpu().numpy().reshape(1, -1)
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_wosam/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_wosam/dist_%s.mat'%(ee)
