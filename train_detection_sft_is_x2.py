...
from hourglass_pose_mask_wj2_x2 import hg
from model_detection_sft_is_x2 import VanishingNet
...
if not os.path.exists('model_x2_run2'):
    os.mkdir('model_x2_run2')
log_path = os.path.join('model_x2_run2', str(datetime.datetime.now()) + '.txt')
mode='train'
import random
random.seed(0)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_x2_run2/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_x2_run2/sdnet_%d.pth' % (epoch + 1)))
