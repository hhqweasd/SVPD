...
from model_vp_conic_anchors_ca_mask_256_test import VanishingNet
import numpy.linalg as LA
import math
from datasets_vp_full_256_test import ImageDataset
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_anchors_ca_mask_256/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_anchors_ca_mask_256/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_anchors_ca_mask_256/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_anchors_ca_mask_256/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_anchors_ca_mask_256/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_anchors_ca_mask_256/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_anchors_ca_mask_256/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_anchors_ca_mask_256/dist_%s.mat'%(ee)
