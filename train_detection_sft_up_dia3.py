...
os.environ["CUDA_VISIBLE_DEVICES"]="1,2,5,0,3,4,6,7"
if not os.path.exists('model_detection_sft_up_dia3'):
    os.mkdir('model_detection_sft_up_dia3')
log_path = os.path.join('model_detection_sft_up_dia3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_detection_sft_up_dia3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_up_dia3/sdnet_%d.pth' % (epoch + 1)))
