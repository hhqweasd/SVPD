...
from model_vp_conic_anchors import VanishingNet
...
if not os.path.exists('model_vp_conic_ori_anchors'):
    os.mkdir('model_vp_conic_ori_anchors')
opt.log_path = os.path.join('model_vp_conic_ori_anchors', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    torch.save(vpnet.state_dict(), 'model_vp_conic_ori_anchors/vpnet.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(vpnet.state_dict(), ('model_vp_conic_ori_anchors/vpnet_%d.pth' % (epoch + 1)))
    ...
    if epoch>90:
        with torch.no_grad(): 
            ...
            if mean_loss < best_mean_loss:
                ...
                torch.save(vpnet.state_dict(), ('model_vp_conic_ori_anchors/vpnet_best.pth'))
