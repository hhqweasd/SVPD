...
os.environ["CUDA_VISIBLE_DEVICES"]="2,3,7,5,0,4,6,1"
...
for ee in range(100,69,-1):
    # ee=92
    generator_1='model_svpnet_run4/vpnet_%s.pth'%(ee)
    generator_2='model_svpnet_run4/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_svpnet_run4/maskis'):
        os.makedirs('model_svpnet_run4/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svpnet_run4/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_svpnet_run4/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svpnet_run4/dist_%s.mat'%(ee)
    ...
    if not os.path.exists('model_svpnet_run4/maskis_data3'):
        os.makedirs('model_svpnet_run4/maskis_data3')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # fake_B448=resize(fake_B448,(256,256,1))
        # outputimage=fake_B448
        # save_result=join_paths('./model_svpnet_run4/maskis_data3/%s'% (str(i) + '.png'))
        # io.imsave(save_result,outputimage)
    ...
    err = np.array(err)
    nnn='model_svpnet_run4/err_data3_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svpnet_run4/dist_data3_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': errdist})
    # exit(0)
