...
from hourglass_pose_mask_wj2_newsft_mask  import hg
...
os.environ["CUDA_VISIBLE_DEVICES"]="2,5,0,1,6,4,7,3"
# os.environ["CUDA_VISIBLE_DEVICES"]="1,3,0,2,5,6,7,4"
if not os.path.exists('model_svp_is_mask_run3'):
    os.mkdir('model_svp_is_mask_run3')
log_path = os.path.join('model_svp_is_mask_run3', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if (epoch+1)>=70:
        torch.save(vpnet.state_dict(), ('model_svp_is_mask_run3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_svp_is_mask_run3/sdnet_%d.pth' % (epoch + 1)))
