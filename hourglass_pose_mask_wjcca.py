...
class SFTLayer(nn.Module): 
     ...
     def forward(self, x, mask): 
        ...
        scale = self.SFT_scale_conv1(F.leaky_relu(self.SFT_scale_conv0(mask), 0.1, inplace=True))
        # shift = self.SFT_shift_conv1(F.leaky_relu(self.SFT_shift_conv0(x[1]), 0.1, inplace=True))
        shift = self.SFT_shift_conv1(F.leaky_relu(self.SFT_shift_conv0(mask), 0.1, inplace=True))
...
class Hourglass(nn.Module):
    ...
    def _hour_glass_forward(self, n, x, mask):
        up1 = self.hg[n - 1][0](x)
        ### Sptial and Channel Feature Transform
        if n==4:
