...
if not os.path.exists('model_ours_lr_run3'):
    os.mkdir('model_ours_lr_run3')
log_path = os.path.join('model_ours_lr_run3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_ours_lr_run3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_ours_lr_run3/sdnet_%d.pth' % (epoch + 1)))
    if epoch == 50:
