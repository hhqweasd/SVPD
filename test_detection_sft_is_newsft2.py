...
os.environ["CUDA_VISIBLE_DEVICES"]="1,0,5,4,3,2,7,6"
...
for ee in range(100,90,-1):
    # ee=92
    generator_1='model_newsft2_run1/vpnet_%s.pth'%(ee)
    generator_2='model_newsft2_run1/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_newsft2_run1_data3/maskis'):
        os.makedirs('model_newsft2_run1_data3/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_newsft2_run1_data3/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_newsft2_run1_data3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_newsft2_run1_data3/dist_%s.mat'%(ee)
