...
class ImageDataset_IS(Dataset):
    def __init__(self, root,mode='test'):
        if mode=='test':
            ...
            self.files_B = sorted(glob.glob(os.path.join(root, '%s/test_B_256_s' % mode) + '/*.*'))
            self.files_C = sorted(glob.glob(os.path.join(root, '%s/test_C' % mode) + '/*.*'))
            self.files_D = sorted(glob.glob(os.path.join(root, '%s/test_B_256' % mode) + '/*.*'))
