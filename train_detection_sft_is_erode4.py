...
from datasetis_erode import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="1,2,3,4,5,6,7,0"
if not os.path.exists('model_detection_sft_is_erode4_run2'):
    os.mkdir('model_detection_sft_is_erode4_run2')
log_path = os.path.join('model_detection_sft_is_erode4_run2', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,mask4,realco) in enumerate(dataloader):
        vpnet.train()
        sdnet.train()
        ...
        loss_sd1=loss_fuse+loss1_h2l+loss2_h2l+loss3_h2l+loss4_h2l+loss1_l2h+loss2_l2h+loss3_l2h+loss4_l2h
        
        
        loss_fuse4 = criterion_GAN(sdmask, mask4)
        loss1_h2l4 = criterion_GAN(s1, mask4)
        loss2_h2l4 = criterion_GAN(s2, mask4)
        loss3_h2l4 = criterion_GAN(s3, mask4)
        loss4_h2l4 = criterion_GAN(s4, mask4)
        loss1_l2h4 = criterion_GAN(s5, mask4)
        loss2_l2h4 = criterion_GAN(s6, mask4)
        loss3_l2h4 = criterion_GAN(s7, mask4)
        loss4_l2h4 = criterion_GAN(s8, mask4)
        
        loss_sd4=loss_fuse4+loss1_h2l4+loss2_h2l4+loss3_h2l4+loss4_h2l4+loss1_l2h4+loss2_l2h4+loss3_l2h4+loss4_l2h4
        
        result=vpnet(real_nsr,realco,sdmask.sigmoid()) 
        loss_vp=_loss(result)
        loss_G=loss_vp+loss_sd1*0.1+loss_sd4
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_erode4_run2/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_erode4_run2/sdnet_%d.pth' % (epoch + 1)))
