...
from datasets_data3 import ImageDataset
...
if not os.path.exists('model_abl_sft1_s_run8'):
    os.mkdir('model_abl_sft1_s_run8')
log_path = os.path.join('model_abl_sft1_s_run8', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
sdnet.load_state_dict(torch.load('vp_data3_3001.pth'))
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    log = 'Epoch: %d,[loss_sd %.5f],[loss_vp %.5f]'%(epoch,avglosssd/i,avglossvp/i)
    avglosssd=0
    avglossvp=0
    print(log)
    open(log_path, 'a').write(log + '\n')
    ...
    if epoch>75:
        torch.save(vpnet.state_dict(), ('model_abl_sft1_s_run8/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_abl_sft1_s_run8/sdnet_%d.pth' % (epoch + 1)))
