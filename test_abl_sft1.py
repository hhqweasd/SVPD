...
os.environ["CUDA_VISIBLE_DEVICES"]="5,1,4,3,2,0,7,6"
...
for ee in range(91,85,-1):
    # ee=92
    generator_1='model_abl_sft1_run2/vpnet_%s.pth'%(ee)
    generator_2='model_abl_sft1_run2/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_abl_sft1_run2/maskis'):
        os.makedirs('model_abl_sft1_run2/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_abl_sft1_run2/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_abl_sft1_run2/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_abl_sft1_run2/dist_%s.mat'%(ee)
