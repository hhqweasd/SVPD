...
from model_vp_conic_anchors_ca_ab_test import VanishingNet
...
os.environ["CUDA_VISIBLE_DEVICES"]="4,3,5,1,2,7,6,0"
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_anchors_ca_ab/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_anchors_ca_ab/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_anchors_ca_ab/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_anchors_ca_ab/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_anchors_ca_ab/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_anchors_ca_ab/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_anchors_ca_ab/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_anchors_ca_ab/dist_%s.mat'%(ee)
