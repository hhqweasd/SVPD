...
from datasets_vp_full_s_256 import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="0,1,5,6,3,4,2,7"
...
if not os.path.exists('model_vp_conic_hgmask_dia3_sd_s'):
    os.mkdir('model_vp_conic_hgmask_dia3_sd_s')
opt.log_path = os.path.join('model_vp_conic_hgmask_dia3_sd_s', str(datetime.datetime.now()) + '.txt')
...
sdnet=BDRAR()
sdnet.load_state_dict(torch.load('3000.pth'))
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        result = vpnet(real_nsr,realco,sdmask)
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_hgmask_dia3_sd_s/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_vp_conic_hgmask_dia3_sd_s/sdnet_%d.pth' % (epoch + 1)))
