...
from conic_convanchor import ConicConv,ConicConv37,ConicConv73
...
class ApolloniusNet(nn.Module):
    ...
    def forward(self, input, vpts,mask):
        ...
        mask=self.pool(mask)
        mask=self.pool(mask)
        m=self.a1*mask+self.b1
        # 128
        x = self.bn1(x)
        xin = self.relu(x)
        
        ## 3x3
        x = self.conv1(xin*m, vpts / self.stride - 0.5)
        x=self.ca11(x)+x
        x = self.pool(x)
        # 64
        x = self.bn2(x)
        ...
        ## 3x7
        x = self.conv12(xin*m, vpts / self.stride - 0.5)
        x=self.ca12(x)+x
        x = self.pool(x)
        # 64
        x = self.bn22(x)
        ...
        ## 7x3
        x = self.conv13(xin*m, vpts / self.stride - 0.5)
        x=self.ca13(x)+x
        x = self.pool(x)
        # 64
        x = self.bn23(x)
