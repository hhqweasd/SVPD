...
from hourglass_pose_mask_wj2_sft import hg
from model_detection_mask_sft_dia3 import VanishingNet
from datasets import ImageDataset
# from datasetis import ImageDataset_IS

os.environ["CUDA_VISIBLE_DEVICES"]="3,4,0,1,2,5,6,7"
if not os.path.exists('model_detection_mask_sft_dia3'):
    os.mkdir('model_detection_mask_sft_dia3')
log_path = os.path.join('model_detection_mask_sft_dia3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
# sdnet1.load_state_dict(torch.load('vp_3001.pth'))
sdnet=sdnet.to(device)
# sdnet1=sdnet.to(device)
sdnet=torch.nn.DataParallel(sdnet,device_ids=[0,1])
# sdnet1=torch.nn.DataParallel(sdnet,device_ids=[0,1])
...
# optimizer_SD1 = torch.optim.Adam(sdnet1.parameters(),lr=opt.lr*0.2,weight_decay=0.0006,amsgrad=True)
...
# dataloader1 = DataLoader(ImageDataset_IS(dataroot,mode),batch_size=opt.batchSize, shuffle=True, num_workers=opt.n_cpu)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        # loss_s_fuse = criterion_GAN(sdmask, mask)
        # loss_s1_h2l = criterion_GAN(s1, smask)
        # loss_s2_h2l = criterion_GAN(s2, smask)
        # loss_s3_h2l = criterion_GAN(s3, smask)
        # loss_s4_h2l = criterion_GAN(s4, smask)
        # loss_s1_l2h = criterion_GAN(s5, smask)
        # loss_s2_l2h = criterion_GAN(s6, smask)
        # loss_s3_l2h = criterion_GAN(s7, smask)
        # loss_s4_l2h = criterion_GAN(s8, smask)
        # loss_s_sd=loss_s_fuse+loss_s1_h2l+loss_s2_h2l+loss_s3_h2l+loss_s4_h2l+loss_s1_l2h+loss_s2_l2h+loss_s3_l2h+loss_s4_l2h
        
        result=vpnet(real_nsr,realco,sdmask.sigmoid()) 
        loss_vp=_loss(result)
        loss_G=loss_vp+loss_sd
        # loss_G=loss_vp+loss_sd+loss_s_sd
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_detection_mask_sft_dia3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_mask_sft_dia3/sdnet_%d.pth' % (epoch + 1)))
