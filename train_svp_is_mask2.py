...
from hourglass_pose_abl_wosft  import hg
from model_detection_sft_is_mask import VanishingNet
...
if not os.path.exists('model_svp_is_mask2_run4'):
    os.mkdir('model_svp_is_mask2_run4')
log_path = os.path.join('model_svp_is_mask2_run4', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
optimizer_SD = torch.optim.Adam(sdnet.parameters(),lr=opt.lr*0.5,weight_decay=0.0006,amsgrad=True)
optimizer_G = torch.optim.Adam(vpnet.parameters(),lr=opt.lr,weight_decay=0.0006,amsgrad=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if (epoch+1)>=70:
        torch.save(vpnet.state_dict(), ('model_svp_is_mask2_run4/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_svp_is_mask2_run4/sdnet_%d.pth' % (epoch + 1)))
