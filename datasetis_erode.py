...
class ImageDataset(Dataset):
    def __init__(self, root,mode='test'):
        self.files_A = sorted(glob.glob(os.path.join(root, '%s/train_A_256' % mode) + '/*.*'))
        self.files_B = sorted(glob.glob(os.path.join(root, '%s/train_B_256' % mode) + '/*.*'))
        self.files_B2 = sorted(glob.glob(os.path.join(root, '%s/train_B_256_erode4' % mode) + '/*.*'))
    ...
    def __getitem__(self, index):
        ...
        item_B2=color.rgb2gray(io.imread(self.files_B2[index % len(self.files_B2)]))
        if k>50:
            item_B=np.fliplr(item_B)
            item_B2=np.fliplr(item_B2)
        if kk>50:            
            item_B=np.flipud(item_B) 
            item_B2=np.flipud(item_B2) 
        item_A=np.asarray(item_A)/255.0
        item_A=torch.from_numpy(item_A.copy()).float()
        item_A=item_A.view(256,256,3)
        item_A=item_A.transpose(0, 1).transpose(0, 2).contiguous()
        
        item_B=np.asarray(item_B)
        item_B=torch.from_numpy(item_B.copy()).float()
        item_B=item_B.view(256,256,1)
        item_B=item_B.transpose(0, 1).transpose(0, 2).contiguous()
        
        item_B2=np.asarray(item_B2)
        item_B2=torch.from_numpy(item_B2.copy()).float()
        item_B2=item_B2.view(256,256,1)
        item_B2=item_B2.transpose(0, 1).transpose(0, 2).contiguous()
        ...
        for eachline in f:
            ...
            if kk>50:            
                x=256.0-x
        f.close()
        item_C=np.array([[x/128-1.0,1.0-y/128,1.0]])# 为什么y取负（宽），因为python的y轴从上到下
        item_C[0] /= LA.norm(item_C[0])
        item_C=torch.from_numpy(item_C.copy()).float()
        return item_A,item_B,item_B2,item_C
        # 图像是否需要归一化？
        # 就不裁剪了，怕把影子减掉
