...
from model_vp_conic_anchors_ca_mask2_test import VanishingNet
...
os.environ["CUDA_VISIBLE_DEVICES"]="3,4,5,1,2,7,6,0"
...
for ee in range(100,91,-1):
    generator_1='model_vp_conic_anchors_ca_mask2/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_anchors_ca_mask2/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_anchors_ca_mask2/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_anchors_ca_mask2/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_anchors_ca_mask2/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_anchors_ca_mask2/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_anchors_ca_mask2/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_anchors_ca_mask2/dist_%s.mat'%(ee)
