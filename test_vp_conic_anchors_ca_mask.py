...
from model_vp_conic_anchors_ca_mask_test import VanishingNet
...
os.environ["CUDA_VISIBLE_DEVICES"]="0,2,5,6,7,1,3,4"
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_anchors_ca_mask_s/vpnet_%s.pth'%(ee)
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_anchors_ca_mask_s/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_anchors_ca_mask_s/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_anchors_ca_mask_s/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_anchors_ca_mask_s/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_anchors_ca_mask_s/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_anchors_ca_mask_s/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_anchors_ca_mask_s/dist_%s.mat'%(ee)
