...
from datasetis128_test import ImageDataset
from hourglass_pose import hg

from model_vp_conic_128_test import VanishingNet

os.environ["CUDA_VISIBLE_DEVICES"]="4,2,5,3,0,7,6,1"
...
for ee in range(100,90,-1):
    generator_1='model_vpnet128/vpnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        vpts_gt[:,0]=(vpts_gt[:,0]+1.0)*64*3024/128
        vpts_gt[:,1]=(1-vpts_gt[:,1])*64*4032/128
        focal_length=4032/2    
        vpts_pd=vpts_pd/vpts_pd[:,2]
        vpts_pd[:,0]=(vpts_pd[:,0]+1)*64*3024/128
        vpts_pd[:,1]=(1-vpts_pd[:,1])*64*4032/128
    ...
    err = np.array(err)
    nnn='model_vpnet128/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vpnet128/dist_%s.mat'%(ee)
