...
from model_vp_conic_anchors_ca_mask import VanishingNet
...
if not os.path.exists('model_vp_conic_anchors_ca_mask_s'):
    os.mkdir('model_vp_conic_anchors_ca_mask_s')
opt.log_path = os.path.join('model_vp_conic_anchors_ca_mask_s', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    # torch.save(vpnet.state_dict(), 'model_vp_conic_anchors_ca_mask_s/vpnet.pth')

    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_anchors_ca_mask_s/vpnet_%d.pth' % (epoch + 1)))
    ...
    if epoch>90:
        with torch.no_grad(): 
            ...
            if mean_loss < best_mean_loss:
                ...
                torch.save(vpnet.state_dict(), ('model_vp_conic_anchors_ca_mask_s/vpnet_best.pth'))
