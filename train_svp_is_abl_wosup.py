...
os.environ["CUDA_VISIBLE_DEVICES"]="0,5,7,1,6,4,2,3"
# os.environ["CUDA_VISIBLE_DEVICES"]="1,3,0,2,5,6,7,4"
if not os.path.exists('model_svp_is_abl_wosup_run2'):
    os.mkdir('model_svp_is_abl_wosup_run2')
log_path = os.path.join('model_svp_is_abl_wosup_run2', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
# sdnet.load_state_dict(torch.load('vp_3001.pth'))
sdnet.load_state_dict(torch.load('3000.pth',map_location=torch.device('cpu')))
...
optimizer_SD = torch.optim.Adam(sdnet.parameters(),lr=opt.lr,weight_decay=0.0006,amsgrad=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        if (i+1) % opt.iter_loss == 0:
            log = '[loss_vp %.5f]'%(avglossvp/i)
            print(log)
            open(log_path, 'a').write(log + '\n')
    # lr_scheduler_SD.step()
    # lr_scheduler_G.step()
    log = 'Epoch: %d,[loss_vp %.5f],[lr_vp %.5f]'%(epoch,avglossvp/i,optimizer_G.param_groups[0]["lr"])
    avglossvp=0
    print(log)
    open(log_path, 'a').write(log + '\n')

    if (epoch+1)>=70:
        torch.save(vpnet.state_dict(), ('model_svp_is_abl_wosup_run2/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_svp_is_abl_wosup_run2/sdnet_%d.pth' % (epoch + 1)))
