...
if not os.path.exists('model_vp_conic_hgmask_s'):
    os.mkdir('model_vp_conic_hgmask_s')
opt.log_path = os.path.join('model_vp_conic_hgmask_s', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_hgmask_s/vpnet_%d.pth' % (epoch + 1)))
