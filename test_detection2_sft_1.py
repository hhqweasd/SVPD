...
# from model_detection_mask_sft_dia3_test import VanishingNet
# from datasets_test import ISImageDataset
# # from datasets import ImageDataset
from hourglass_pose_mask_IS_sft_1 import hg
from model_detection2_sft_1_test import VanishingNet
from datasets_test import ImageDataset_IS

os.environ["CUDA_VISIBLE_DEVICES"]="3,2,5,0,4,7,6,1"
...
for ee in range(100,90,-1):
    generator_1='model_detection2_sft_1_run2/vpnet_%s.pth'%(ee)
    generator_2='model_detection2_sft_1_run2/sdnet_%s.pth'%(ee)
    generator_3='model_detection2_sft_1_run2/sdnet_IS_%s.pth'%(ee)
    vpnet.load_state_dict(torch.load(generator_1))
    vpnet.eval()
    sdnet.load_state_dict(torch.load(generator_2))
    sdnet.eval()
    sdnet_IS.load_state_dict(torch.load(generator_3))
    sdnet_IS.eval()
    ...
    for i, (real_nsr,mask,realco,mask_IS) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection2_sft_1_run2/mask/%s'% (str(i) + '.png'))
        io.imsave(save_result,outputimage)
        fake_B448 = sdmask_IS.data
        fake_B448=255.0*(fake_B448+1.0)
        fake_B448=fake_B448.data.squeeze(0).cpu()
        fake_B448=fake_B448.transpose(0, 2).transpose(0, 1).contiguous().numpy()
        fake_B448=resize(fake_B448,(256,256,1))
        outputimage=fake_B448
        save_result=join_paths('./model_detection2_sft_1_run2/maskis/%s'% (str(i) + '.png'))
        io.imsave(save_result,outputimage)
        '''
        vpts_gt = realco[0]
        ...
        with torch.no_grad():
            score = vpnet(real_nsr,vpts,sdmask,sdmask_IS)[:, -1].cpu().numpy()
        ...
        for res in range(1, len(multires)):
            ...
            vps = np.vstack(vpts)
            with torch.no_grad():
                score = vpnet(real_nsr,vps,sdmask,sdmask_IS)[:, -res - 1].cpu().numpy().reshape(1, -1)
    ...
    err = np.array(err)
    nnn='model_detection2_sft_1_run2/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection2_sft_1_run2/dist_%s.mat'%(ee)
