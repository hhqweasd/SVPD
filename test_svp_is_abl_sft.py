...
os.environ["CUDA_VISIBLE_DEVICES"]="3,0,7,2,4,5,6,1"
...
for ee in range(94,69,-1):
    # ee=92
    generator_1='model_svp_is_abl_sft_run7/vpnet_%s.pth'%(ee)
    generator_2='model_svp_is_abl_sft_run7/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_svp_is_abl_sft_run7/maskis'):
        os.makedirs('model_svp_is_abl_sft_run7/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_is_abl_sft_run7/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_svp_is_abl_sft_run7/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_abl_sft_run7/dist_%s.mat'%(ee)
