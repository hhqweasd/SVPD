...
os.environ["CUDA_VISIBLE_DEVICES"]="0,1,5,2,3,4,6,7"
if not os.path.exists('model_newsft_run1'):
    os.mkdir('model_newsft_run1')
log_path = os.path.join('model_newsft_run1', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_newsft_run1/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_newsft_run1/sdnet_%d.pth' % (epoch + 1)))
