...
os.environ["CUDA_VISIBLE_DEVICES"]="0,2,7,4,1,5,6,3"
...
for ee in range(100,69,-1):
    # ee=92
    generator_1='model_svp_is_abl_wop_run2/vpnet_%s.pth'%(ee)
    generator_2='model_svp_is_abl_wop_run2/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_svp_is_abl_wop_run2/maskis'):
        os.makedirs('model_svp_is_abl_wop_run2/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_is_abl_wop_run2/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_svp_is_abl_wop_run2/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_abl_wop_run2/dist_%s.mat'%(ee)
    ...
    if not os.path.exists('model_svp_is_abl_wop_run2/maskis_data3'):
        os.makedirs('model_svp_is_abl_wop_run2/maskis_data3')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # fake_B448=resize(fake_B448,(256,256,1))
        # outputimage=fake_B448
        # save_result=join_paths('./model_svp_is_abl_wop_run2/maskis_data3/%s'% (str(i) + '.png'))
        # io.imsave(save_result,outputimage)
    ...
    err = np.array(err)
    nnn='model_svp_is_abl_wop_run2/err_data3_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_abl_wop_run2/dist_data3_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': errdist})
