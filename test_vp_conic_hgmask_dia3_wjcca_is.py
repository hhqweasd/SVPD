...
from hourglass_pose_mask_wjcca import hg
from model_vp_conic_hgmask_dia3_test import VanishingNet
import numpy.linalg as LA
import math
from datasets_vp_full_is_test import ImageDataset
...
os.environ["CUDA_VISIBLE_DEVICES"]="1,2,5,0,4,7,6,3"
...
for ee in range(100,90,-1):
    generator_1='model_vp_conic_hgmask_dia3_wjcca_is/vpnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco,name) in enumerate(dataloader):
        ...
        vpts_gt[:,0]=(vpts_gt[:,0]+1.0)*256*3024/512
        vpts_gt[:,1]=(1-vpts_gt[:,1])*256*4032/512
        focal_length=4032/2    
        vpts_pd=vpts_pd/vpts_pd[:,2]
        vpts_pd[:,0]=(vpts_pd[:,0]+1)*256*3024/512
        vpts_pd[:,1]=(1-vpts_pd[:,1])*256*4032/512
    ...
    err = np.array(err)
    np.savez('./model_vp_conic_hgmask_dia3_wjcca_is/angle_best.npz', err=err)
    scipyio.savemat('model_vp_conic_hgmask_dia3_wjcca_is/err_best.mat', {'name': err})
    errdist = np.array(errdist)
    np.savez('./model_vp_conic_hgmask_dia3_wjcca_is/dist_best.npz', errdist=errdist)
    scipyio.savemat('model_vp_conic_hgmask_dia3_wjcca_is/dist_best.mat', {'name': errdist})
    scipyio.savemat('model_vp_conic_hgmask_dia3_wjcca_is/imgname_best.mat', {'name': imgname})
    '''
    
    err = np.array(err)
    nnn='model_vp_conic_hgmask_dia3_wjcca_is/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vp_conic_hgmask_dia3_wjcca_is/dist_%s.mat'%(ee)
