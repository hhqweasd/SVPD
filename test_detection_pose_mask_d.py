...
from model_detection_pose_mask_d_test import VanishingNet
...
for ee in range(100,90,-1):
    generator_1='model_detection_pose_mask_d/vpnet_%s.pth'%(ee)
    generator_2='model_detection_pose_mask_d/sdnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        save_result='./model_detection_pose_mask_d/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection_pose_mask_d/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_pose_mask_d/dist_%s.mat'%(ee)
