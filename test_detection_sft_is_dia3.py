...
from model_detection_sft_is_dia3_test import VanishingNet
...
for ee in range(100,90,-1):
    generator_1='model_detection_sft_is_dia3/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_dia3/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_dia3/maskis'):
        os.makedirs('model_detection_sft_is_dia3/maskis')
    ...
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection_sft_is_dia3/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_dia3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_dia3/dist_%s.mat'%(ee)
