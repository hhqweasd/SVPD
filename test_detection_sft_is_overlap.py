...
mode='test'
dataroot = '/home/liuzhihao/dataset/vpdataset-overlap5p'
...
for ee in range(100,90,-1):
    ...
    for i, (real_nsr,realco,name) in enumerate(dataloader):
        '''
    ...
    '''
    err = np.array(err)
    nnn='model_detection_sft_is_run3_18/err_overlap5p_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_run3_18/dist_overlap5p_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': errdist})
    '''
    imname = np.array(imname)
    print(imname)
    nnn='model_detection_sft_is_run3_18/image_overlap5p_%s.mat'%(ee)
