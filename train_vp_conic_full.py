...
from model_vp_conic_full import VanishingNet

os.environ["CUDA_VISIBLE_DEVICES"]="0,4,1,2,3,7,5,6"
...
if not os.path.exists('model_vp_conic_full'):
    os.mkdir('model_vp_conic_full')
opt.log_path = os.path.join('model_vp_conic_full', str(datetime.datetime.now()) + '.txt')
...
#optimizer_G = torch.optim.Adam(itertools.chain(vpnet.parameters(), isdnet.parameters()),lr=opt.lr*2,weight_decay=0.0006,amsgrad=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        result,fakemask = vpnet(real_nsr,mask,realco)
        loss_mask=criterion_GAN(fakemask,mask)
        loss_identity_B = _loss(result)
        if np.isnan(loss_identity_B.item()):
            raise ValueError("loss is nan while training")
            
        loss_G=loss_identity_B+loss_mask
        loss_G.backward()
        ...
        if (i+1) % opt.iter_loss == 0:
            log = 'Epoch: %d, [iter %d], [loss_identity_B %.5f], [loss_mask %.5f]' % \
                  (epoch, curr_iter, loss_identity_B,loss_mask)
            print(log)
    ...
    torch.save(vpnet.state_dict(), 'model_vp_conic_full/vpnet.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(vpnet.state_dict(), ('model_vp_conic_full/vpnet_%d.pth' % (epoch + 1)))
