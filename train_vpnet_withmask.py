...
os.environ["CUDA_VISIBLE_DEVICES"]="3,4,0,5,2,6,7,1"
if not os.path.exists('model_vpnet_withmask'):
    os.mkdir('model_vpnet_withmask')
log_path = os.path.join('model_vpnet_withmask', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask_IS,realco) in enumerate(dataloader):
        ...
        result=vpnet(torch.cat((real_nsr,mask_IS),1),realco) 
    ...
    if epoch>(opt.n_epochs-11):
        torch.save(vpnet.state_dict(), ('model_vpnet_withmask/vpnet_%d.pth' % (epoch + 1)))
