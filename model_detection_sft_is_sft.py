...
class VanishingNet(nn.Module):
    ...
    def forward(self, x,vps,mask):
        ...
        mask = mask[:, None].repeat(1, c, 1, 1, 1).reshape(N * c, 1, int(mask.size(2)), int(mask.size(3)))
...
class SFTLayer(nn.Module): 
     def __init__(self,k): 
         ...
         self.SFT_scale_conv0 = nn.Conv2d(1, k, 1) 
         self.SFT_scale_conv1 = nn.Conv2d(k, 2*k, 1) 
         self.SFT_scale_conv2 = nn.Conv2d(2*k, k, 1)
         self.SFT_shift_conv0 = nn.Conv2d(1, k, 1) 
         self.SFT_shift_conv1 = nn.Conv2d(k, 2*k, 1) 
         self.SFT_shift_conv2 = nn.Conv2d(2*k, k, 1) 
...
class ApolloniusNet(nn.Module):
    def __init__(self, output_stride, upsample_scale):
        ...
        self.sft2 = SFTLayer(64)
        self.sft3 = SFTLayer(128)
        self.sft4 = SFTLayer(256)
    ...
    def forward(self, input, vpts,mask):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        mask = F.interpolate(mask, [int(x.size(2)),int(x.size(3))], mode='bilinear', align_corners=True)
        x = self.sft1(x, mask)
