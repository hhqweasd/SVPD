...
from hourglass_pose_mask_wj2_abl_3 import hg
from model_detection_sft_is import VanishingNet
# from datasets import ImageDataset
from datasetis import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="3,4,0,2"
if not os.path.exists('model_detection_sft_is_abl_3'):
    os.mkdir('model_detection_sft_is_abl_3')
log_path = os.path.join('model_detection_sft_is_abl_3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_abl_3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_abl_3/sdnet_%d.pth' % (epoch + 1)))
