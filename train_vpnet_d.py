...
os.environ["CUDA_VISIBLE_DEVICES"]="1,2,4,5,6,7,3,0"
if not os.path.exists('model_vpnet_d'):
    os.mkdir('model_vpnet_d')
log_path = os.path.join('model_vpnet_d', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vpnet_d/vpnet_%d.pth' % (epoch + 1)))
