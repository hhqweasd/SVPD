...
for ee in range(100,90,-1):
    ee=95
    generator_1='model_vpnet/vpnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_vpnet/maskis-2'):
        os.makedirs('model_vpnet/maskis-2')
    ...
    err = np.array(err)
    nnn='model_vpnet/err-2_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vpnet/dist-2_%s.mat'%(ee)
