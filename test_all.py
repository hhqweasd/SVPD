from __future__ import print_function
import os
import torchvision
import torch
import numpy as np
from skimage import io
from skimage.transform import resize
import numpy.linalg as LA
import math
from torch.utils.data import DataLoader
import scipy.io as scipyio
from os.path import exists, join as join_paths

import matplotlib.pyplot as plt
from bdrar import BDRAR
from hourglass_pose_mask_wj2_newsft import hg
from model_detection_sft_is_test import VanishingNet
# from datasets_test import ImageDataset
from datasetis_test import ImageDataset

from hourglass_pose import hg as hg_1

from model_vp_conic_test_256 import VanishingNet as VanishingNet_1


os.environ["CUDA_VISIBLE_DEVICES"]="0,3,7,2,1,5,6,4"

multires=[0.0051941870036646, 0.02004838034795, 0.0774278195486317, 0.299564810864565]
def sample_sphere(v, alpha, num_pts):
    v1 = orth(v)
    v2 = np.cross(v, v1)
    v, v1, v2 = v[:, None], v1[:, None], v2[:, None]
    indices = np.linspace(1, num_pts, num_pts)
    phi = np.arccos(1 + (math.cos(alpha) - 1) * indices / num_pts)
    theta = np.pi * (1 + 5 ** 0.5) * indices
    r = np.sin(phi)
    return (v * np.cos(phi) + r * (v1 * np.cos(theta) + v2 * np.sin(theta))).T
def orth(v):
    x, y, z = v
    o = np.array([0.0, -z, y] if abs(x) < abs(y) else [-z, 0.0, x])
    o /= LA.norm(o)
    return o

mode='test'
dataroot = '/home/liuzhihao/dataset/SVPI'
device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
sdnet=BDRAR()
sdnet=sdnet.to(device)
sdnet=torch.nn.DataParallel(sdnet,device_ids=[0,1])
vpnet=hg(planes=64, depth=4, num_stacks=1, num_blocks=1)
vpnet=VanishingNet(vpnet,4,1)
vpnet=vpnet.to(device)
vpnet=torch.nn.DataParallel(vpnet,device_ids=[0,1])

vpnet_1=hg_1(planes=64, depth=4, num_stacks=1, num_blocks=1)
vpnet_1=VanishingNet_1(vpnet_1,4,1)
vpnet_1=vpnet_1.to(device)
vpnet_1=torch.nn.DataParallel(vpnet_1,device_ids=[0,1])

generator_1='model_svp_is_run2/vpnet_%s.pth'%(77)
generator_2='model_svp_is_run2/sdnet_%s.pth'%(77)
vpnet.load_state_dict(torch.load(generator_1))
vpnet.eval()
sdnet.load_state_dict(torch.load(generator_2))
sdnet.eval()

generator_3='model_vpnet_data3_run1/vpnet_100.pth'
vpnet_1.load_state_dict(torch.load(generator_3))

err = []
errdist = []
dataloader=DataLoader(ImageDataset(dataroot,mode),batch_size=1,shuffle=False,num_workers=2)

if not os.path.exists('model_svp_is_run2/result'):
    os.makedirs('model_svp_is_run2/result')
    
for image_index, (real_nsr,realco) in enumerate(dataloader):
    temp_nsr=real_nsr.cuda()
    real_nsr=real_nsr.cuda()
    sdmask=sdnet(real_nsr)   
 
    vpts_gt = realco[0]
    vpts_gt *= (vpts_gt[:, 2:3] > 0).float() * 2 - 1
    vpts = sample_sphere(np.array([0, 0, 1]), np.pi / 2, 64)
    with torch.no_grad():
        score = vpnet(real_nsr,vpts,sdmask)[:, -1].cpu().numpy()
    index = np.argsort(-score)
    candidate = [index[0]]
    for i in index[1:]:
        if len(candidate) == 1:
            break
        dst = np.min(np.arccos(np.abs(vpts[candidate] @ vpts[i])))
        if dst < np.pi / 1:
            continue
        candidate.append(i)
    vpts_pd = vpts[candidate]

    for res in range(1, len(multires)):
        vpts = [sample_sphere(vpts_pd[vp], multires[-res], 64) for vp in range(1)]
        vps = np.vstack(vpts)
        with torch.no_grad():
            score = vpnet(real_nsr,vps,sdmask)[:, -res - 1].cpu().numpy().reshape(1, -1)
        for i, s in enumerate(score):
            vpts_pd[i] = vpts[i][np.argmax(s)]
    
    
    vpts_gt=vpts_gt/vpts_gt[:,2]
    vpts_gt[:,0]=(vpts_gt[:,0]+1.0)*128*3024/256
    vpts_gt[:,1]=(1-vpts_gt[:,1])*128*4032/256
    vpts_pd=vpts_pd/vpts_pd[:,2]
    vpts_pd[:,0]=(vpts_pd[:,0]+1)*128*3024/256
    vpts_pd[:,1]=(1-vpts_pd[:,1])*128*4032/256

    
    real_nsr=real_nsr.data
    real_nsr=real_nsr
    real_nsr=real_nsr.data.squeeze(0).cpu()
    real_nsr=real_nsr.transpose(0, 2).transpose(0, 1).contiguous().numpy()
    real_nsr=resize(real_nsr,(378,504,3))
    
    plt.figure(dpi=300)
    plt.axis([-504,1008,378,0])
    plt.axis('off')
    plt.imshow(real_nsr)
    plt.scatter(vpts_gt[0,1]*504/4032,vpts_gt[0,0]*378/3024,color='red',s=20)
    plt.scatter(vpts_pd[0,1]*504/4032,vpts_pd[0,0]*378/3024,color='darkorange',s=20)
    
    
    real_nsr=temp_nsr
    vpts_gt = realco[0]
    vpts_gt *= (vpts_gt[:, 2:3] > 0).float() * 2 - 1
    vpts = sample_sphere(np.array([0, 0, 1]), np.pi / 2, 64)
    with torch.no_grad():
        score = vpnet_1(real_nsr,vpts)[:, -1].cpu().numpy()
    index = np.argsort(-score)
    candidate = [index[0]]
    for i in index[1:]:
        if len(candidate) == 1:
            break
        dst = np.min(np.arccos(np.abs(vpts[candidate] @ vpts[i])))
        if dst < np.pi / 1:
            continue
        candidate.append(i)
    vpts_pd = vpts[candidate]

    for res in range(1, len(multires)):
        vpts = [sample_sphere(vpts_pd[vp], multires[-res], 64) for vp in range(1)]
        vps = np.vstack(vpts)
        with torch.no_grad():
            score = vpnet_1(real_nsr,vps)[:, -res - 1].cpu().numpy().reshape(1, -1)
        for i, s in enumerate(score):
            vpts_pd[i] = vpts[i][np.argmax(s)]
    
    for vp in vpts_gt.numpy():
        angle=min(np.arccos(np.abs(vpts_pd @ vp).clip(max=1))) / np.pi * 180
        err.append(angle)


    vpts_pd=vpts_pd/vpts_pd[:,2]
    vpts_pd[:,0]=(vpts_pd[:,0]+1)*128*3024/256
    vpts_pd[:,1]=(1-vpts_pd[:,1])*128*4032/256

    plt.scatter(vpts_pd[0,1]*504/4032,vpts_pd[0,0]*378/3024,color='fuchsia',s=20)
    save_result=join_paths('./model_svp_is_run2/result/%s'% (str(image_index) + '.jpg')) 
    # plt.savefig(save_result, format='jpg', transparent=True, pad_inches=0)
    plt.savefig(save_result, format='jpg', transparent=True, pad_inches=0, bbox_inches='tight')
    plt.close('all')
    
