...
os.environ["CUDA_VISIBLE_DEVICES"]="3,5,7,6,4,2,0,1"
...
mode='test'
dataroot = '/home/liuzhihao/dataset/vpdataset-2'
...
for ee in range(100,90,-1):
    ...
    if not os.path.exists('model_detection_sft_is_run3_18/maskis-2'):
        os.makedirs('model_detection_sft_is_run3_18/maskis-2')
    ...
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection_sft_is_run3_18/maskis-2/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_run3_18/err-2_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_run3_18/dist-2_%s.mat'%(ee)
