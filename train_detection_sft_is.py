...
os.environ["CUDA_VISIBLE_DEVICES"]="7,6,0,1,5,2,3,4"
if not os.path.exists('model_detection_sft_is_data3_run3'):
    os.mkdir('model_detection_sft_is_data3_run3')
log_path = os.path.join('model_detection_sft_is_data3_run3', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_data3_run3/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_data3_run3/sdnet_%d.pth' % (epoch + 1)))
