...
from model_detection2_sft_dia3_test import VanishingNet
from datasets_test import ImageDataset_IS

os.environ["CUDA_VISIBLE_DEVICES"]="0,1,5,3,4,7,6,2"
...
for ee in range(100,90,-1):
    generator_1='model_detection2_sft_dia3/vpnet_%s.pth'%(ee)
    generator_2='model_detection2_sft_dia3/sdnet_%s.pth'%(ee)
    generator_3='model_detection2_sft_dia3/sdnet_IS_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco,mask_IS) in enumerate(dataloader):
        ...
        save_result='./model_detection2_sft_dia3/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection2_sft_dia3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection2_sft_dia3/dist_%s.mat'%(ee)
