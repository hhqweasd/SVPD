...
from model_vp_conic_attwomask_test import VanishingNet
...
os.environ["CUDA_VISIBLE_DEVICES"]="1,2,3,0,7,5,6,4"
...
parser.add_argument('--generator_1', type=str, default='model_vp_conic_attwomask_dia3/vpnet_95.pth', help='A2B generator checkpoint file')
...
for i, (real_nsr,mask,realco,name) in enumerate(dataloader):
    ...
    #mask=mask.cuda()
    #real_nsr=torch.cat((real_nsr,real_nsr*mask),1)
    vpts_gt = realco[0]
    vpts_gt *= (vpts_gt[:, 2:3] > 0).float() * 2 - 1
    vpts = sample_sphere(np.array([0, 0, 1]), np.pi / 2, 64)
    with torch.no_grad():
        score = vpnet(real_nsr,mask,vpts)[:, -1].cpu().numpy()
    ...
    for res in range(1, len(multires)):
        ...
        vps = np.vstack(vpts)
        with torch.no_grad():
            score = vpnet(real_nsr,mask,vps)[:, -res - 1].cpu().numpy().reshape(1, -1)
...
err = np.array(err)
np.savez('./model_vp_conic_attwomask_dia3/angle_95.npz', err=err)
scipyio.savemat('model_vp_conic_attwomask_dia3/err_95.mat', {'name': err})
errdist = np.array(errdist)
np.savez('./model_vp_conic_attwomask_dia3/dist_95.npz', errdist=errdist)
scipyio.savemat('model_vp_conic_attwomask_dia3/dist_95.mat', {'name': errdist})
scipyio.savemat('model_vp_conic_attwomask_dia3/imgname_95.mat', {'name': imgname})
