...
class Hourglass(nn.Module):
    ...
    def _hour_glass_forward(self, n, x, mask, IS_mask ):
        # print(mask.size())
        # print(IS_mask.size())
        up1 = self.hg[n - 1][0](x)
        if n==4:
            mask=self.maxpool(mask)
            IS_mask=self.maxpool(IS_mask)
            IS_mask=self.maxpool(IS_mask)
        # upmask = self.hgmask[n - 1][0](x*mask)
        low1 = F.max_pool2d(x, 2, stride=2)
        mask = F.max_pool2d(mask, 2, stride=2)
        IS_mask = F.max_pool2d(IS_mask, 2, stride=2)
        if n==4:
            low1 = self.sft4(low1, mask,IS_mask)
            # low1 = self.cca(low1)
        elif n==3:
            low1 = self.sft3(low1, mask,IS_mask)
        elif n==2:
            low1 = self.sft2(low1, mask,IS_mask)
        else:
            low1 = self.sft1(low1, mask,IS_mask)
        low1 = self.hg[n - 1][1](low1)

        if n > 1:
            low2 = self._hour_glass_forward(n - 1, low1,mask,IS_mask)
        else:
    ...
    def forward(self, x,mask,IS_mask):
        return self._hour_glass_forward(self.depth, x, mask,IS_mask)
