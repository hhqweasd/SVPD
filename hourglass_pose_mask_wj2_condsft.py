...
class SFTLayer(nn.Module): 
     def __init__(self,inp,oup): 
         ...
         self.SFT_scale_conv0 = nn.Conv2d(inp, inp, 1) 
         self.SFT_scale_conv1 = nn.Conv2d(inp, oup , 1) 
         self.SFT_shift_conv0 = nn.Conv2d(inp, inp, 1) 
         self.SFT_shift_conv1 = nn.Conv2d(inp, oup, 1) 
...
class Hourglass(nn.Module):
    def __init__(self, block, num_blocks, planes, depth):
        ...
        self.sft1 = SFTLayer(128,256)
        self.sft2 = SFTLayer(128,256)
        self.sft3 = SFTLayer(128,256)
        self.sft4 = SFTLayer(128,256)
    ...
    def _hour_glass_forward(self, n, x, cond):
        up1 = self.hg[n - 1][0](x)
        if n==4:
            low1 = self.sft4(x, cond)
            cond = F.max_pool2d(cond, 2, stride=2)
        elif n==3:
            low1 = self.sft3(x, cond)
            cond = F.max_pool2d(cond, 2, stride=2)
        elif n==2:
            low1 = self.sft2(x, cond)
            cond = F.max_pool2d(cond, 2, stride=2)
        else:
            low1 = self.sft1(x, cond)
        low1 = F.max_pool2d(x, 2, stride=2)
        low1 = self.hg[n - 1][1](low1)

        if n > 1:
            low2 = self._hour_glass_forward(n - 1, low1,cond)
        else:
