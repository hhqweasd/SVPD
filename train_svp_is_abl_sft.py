...
from hourglass_pose_mask_wj2_newsft4  import hg
...
os.environ["CUDA_VISIBLE_DEVICES"]="1,4,3,5,2,6,7,0"
if not os.path.exists('model_svp_is_abl_sft_run8'):
    os.mkdir('model_svp_is_abl_sft_run8')
log_path = os.path.join('model_svp_is_abl_sft_run8', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if (epoch+1)>=70:
        torch.save(vpnet.state_dict(), ('model_svp_is_abl_sft_run8/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_svp_is_abl_sft_run8/sdnet_%d.pth' % (epoch + 1)))
