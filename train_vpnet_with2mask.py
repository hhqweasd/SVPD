...
os.environ["CUDA_VISIBLE_DEVICES"]="2,3,0,4,5,6,7,1"
if not os.path.exists('model_vpnet_with2mask'):
    os.mkdir('model_vpnet_with2mask')
log_path = os.path.join('model_vpnet_with2mask', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco, mask_IS) in enumerate(dataloader):
        ...
        result=vpnet(torch.cat((real_nsr,mask,mask_IS),1),realco) 
        ...
        if (i+1) % opt.iter_loss == 0:
            log = 'Epoch: %d,[loss_vp %.5f]'%(epoch,loss_vp,)
            print(log)
            open(log_path, 'a').write(log + '\n')

    if epoch>(opt.n_epochs-11):
        torch.save(vpnet.state_dict(), ('model_vpnet_with2mask/vpnet_%d.pth' % (epoch + 1)))
