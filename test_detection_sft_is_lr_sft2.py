...
os.environ["CUDA_VISIBLE_DEVICES"]="2,5,0,4,3,1,7,6"
...
for ee in range(100,75,-1):
    # ee=92
    generator_1='model_new_sft2_lr_run5/vpnet_%s.pth'%(ee)
    generator_2='model_new_sft2_lr_run5/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_new_sft2_lr_run5/maskis'):
        os.makedirs('model_new_sft2_lr_run5/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_new_sft2_lr_run5/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_new_sft2_lr_run5/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_new_sft2_lr_run5/dist_%s.mat'%(ee)
