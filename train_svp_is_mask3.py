...
from bdrar_2 import BDRAR
from model_detection_sft_is_mask3 import VanishingNet
from datasetis import ImageDataset

import warnings
warnings.filterwarnings("ignore")

# os.environ["CUDA_VISIBLE_DEVICES"]="0,1,5,6,7,4,2,3"
# os.environ["CUDA_VISIBLE_DEVICES"]="2,3,0,1,5,6,7,4"
# os.environ["CUDA_VISIBLE_DEVICES"]="4,5,0,1,6,7,2,3"
os.environ["CUDA_VISIBLE_DEVICES"]="6,7,0,1,5,4,2,3"
# os.environ["CUDA_VISIBLE_DEVICES"]="5,3,0,1,6,4,2,7"
# os.environ["CUDA_VISIBLE_DEVICES"]="2,4,0,1,5,6,7,3"
if not os.path.exists('model_svp_is_mask3_run2'):
    os.mkdir('model_svp_is_mask3_run2')
log_path = os.path.join('model_svp_is_mask3_run2', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
sdnet=BDRAR()
# sdnet.load_state_dict(torch.load('vp_3001.pth'))
# sdnet.load_state_dict(torch.load('vp_3001.pth'))
# sdnet.load_state_dict(torch.load('vp_new_data3_3001.pth',map_location=torch.device('cpu')))
...
optimizer_SD = torch.optim.Adam(sdnet.parameters(),lr=0.0005,weight_decay=0.0006,amsgrad=True)
optimizer_G = torch.optim.Adam(vpnet.parameters(),lr=0.0002,weight_decay=0.0006,amsgrad=True)

# lr_scheduler_SD = torch.optim.lr_scheduler.LambdaLR(optimizer_SD,lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, opt.decay_epoch).step)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        sdmask,s1,s2,s3,s4,s5,s6,s7,s8,feature_vp=sdnet(real_nsr)             
        ...
        result=vpnet(feature_vp,realco,sdmask.sigmoid()) 
        ...
        if (i+1) % opt.iter_loss == 0:
            log = '[loss_sd %.5f],[loss_vp %.5f]'%(avglosssd/i,avglossvp/i)
            print(log)
            open(log_path, 'a').write(log + '\n')

    log = 'Epoch: %d,[loss_sd %.5f],[loss_vp %.5f],[lr_vp %.5f]'%(epoch,avglosssd/i,avglossvp/i,optimizer_G.param_groups[0]["lr"])
    avglosssd=0
    avglossvp=0
    print(log)
    open(log_path, 'a').write(log + '\n')
    ...
    if (epoch+1)>=70:
        torch.save(vpnet.state_dict(), ('model_svp_is_mask3_run2/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_svp_is_mask3_run2/sdnet_%d.pth' % (epoch + 1)))

    if epoch == 60:
        optimizer_G.param_groups[0]["lr"] /= 2
        optimizer_SD.param_groups[0]["lr"] /= 2
    if epoch == 70:
        optimizer_G.param_groups[0]["lr"] /= 2
        optimizer_SD.param_groups[0]["lr"] /= 2
    if epoch == 80:
