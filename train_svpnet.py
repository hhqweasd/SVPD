...
from hourglass_pose_mask_wj2_condsft  import hg
...
if not os.path.exists('model_svpnet_run4'):
    os.mkdir('model_svpnet_run4')
log_path = os.path.join('model_svpnet_run4', str(datetime.datetime.now()) + '.txt')
mode='train_data3'
...
# sdnet.load_state_dict(torch.load('vp_new_data3_3001.pth'))
sdnet.load_state_dict(torch.load('vp_3001.pth',map_location=torch.device('cpu')))
# sdnet.load_state_dict(torch.load('vp_shadow_data3_3001.pth'))
# sdnet.load_state_dict(torch.load('vp_3001.pth'),map_location=torch.device('cpu'))
...
optimizer_SD = torch.optim.Adam(sdnet.parameters(),lr=opt.lr*0.1,weight_decay=0.0006,amsgrad=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if (epoch+1)>70:
        torch.save(vpnet.state_dict(), ('model_svpnet_run4/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_svpnet_run4/sdnet_%d.pth' % (epoch + 1)))
