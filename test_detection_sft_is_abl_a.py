...
from hourglass_pose_mask_wj2_abl_a import hg
...
os.environ["CUDA_VISIBLE_DEVICES"]="4,1,7,6,2,3,0,5"
...
for ee in range(100,90,-1):
    ee=92
    generator_1='model_detection_sft_is_a/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_a/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_a_data3'):
        os.makedirs('model_detection_sft_is_a_data3')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        save_result=join_paths('./model_detection_sft_is_a_data3/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_a_data3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_a_data3/dist_%s.mat'%(ee)
