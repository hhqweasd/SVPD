...
from hourglass_pose_mask_wj2_abl_wosft import hg
...
dataroot_data3 = '/home/liuzhihao/dataset/vpdataset-data3' # 只有data3的测试集合，共67个
# dataroot = '/home/liuzhihao/dataset/vpdatasetdata3'  # 所有data3当做测试集，共400个
...
for ee in range(100,69,-1):
    # ee=92
    generator_1='model_svp_is_abl_wosft_run4/vpnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_svp_is_abl_wosft_run4/maskis'):
        os.makedirs('model_svp_is_abl_wosft_run4/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_svp_is_abl_wosft_run4/maskis/%s'% (str(i) + '.png'))
        ...
        with torch.no_grad():
            score = vpnet(real_nsr,vpts,real_nsr)[:, -1].cpu().numpy()
        ...
        for res in range(1, len(multires)):
            ...
            vps = np.vstack(vpts)
            with torch.no_grad():
                score = vpnet(real_nsr,vps,real_nsr)[:, -res - 1].cpu().numpy().reshape(1, -1)
    ...
    err = np.array(err)
    nnn='model_svp_is_abl_wosft_run4/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_abl_wosft_run4/dist_%s.mat'%(ee)
    ...
    print(ee)
    print('data12')
    print(np.mean(errdist))
    
    err = []
    errdist = []
    dataloader=DataLoader(ImageDataset(dataroot_data3,mode),batch_size=1,shuffle=False,num_workers=2)

    if not os.path.exists('model_svp_is_abl_wosft_run4/maskis_data3'):
        os.makedirs('model_svp_is_abl_wosft_run4/maskis_data3')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # fake_B448=resize(fake_B448,(256,256,1))
        # outputimage=fake_B448
        # save_result=join_paths('./model_svp_is_abl_wosft_run4/maskis_data3/%s'% (str(i) + '.png'))
        # io.imsave(save_result,outputimage)
    ...
    err = np.array(err)
    nnn='model_svp_is_abl_wosft_run4/err_data3_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_svp_is_abl_wosft_run4/dist_data3_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': errdist})
