...
from model_detection_sft_is_4loss import VanishingNet
...
os.environ["CUDA_VISIBLE_DEVICES"]="2,3,7,6,5,0,1,4"
if not os.path.exists('model_detection_sft_is_4loss_run1'):
    os.mkdir('model_detection_sft_is_4loss_run1')
log_path = os.path.join('model_detection_sft_is_4loss_run1', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>84:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_4loss_run1/vpnet_%d.pth' % (epoch + 1)))
        torch.save(sdnet.state_dict(), ('model_detection_sft_is_4loss_run1/sdnet_%d.pth' % (epoch + 1)))
