...
from model_detection_sft_test import VanishingNet
from datasets_test import ImageDataset
# from datasets import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="7,4,0,2,3,5,6,1"
...
for ee in range(100,90,-1):
    ee=99
    generator_1='model_detection_sft/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_data3'):
        os.makedirs('model_detection_sft_data3')
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        save_result='./model_detection_sft/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection_sft_data3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_data3/dist_%s.mat'%(ee)
