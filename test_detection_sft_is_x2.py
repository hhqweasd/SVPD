...
from model_detection_sft_is_x2_test import VanishingNet
...
vpnet=VanishingNet(vpnet,2,1)
vpnet=vpnet.to(device)
vpnet=torch.nn.DataParallel(vpnet,device_ids=[0,1])

for ee in range(100,90,-1):
    # ee=92
    generator_1='model_x2_run2/vpnet_%s.pth'%(ee)
    generator_2='model_x2_run2/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_x2_run2_data3/maskis'):
        os.makedirs('model_x2_run2_data3/maskis')
    # if not os.path.exists('model_x2_run2_data3/maskis'):
        # os.makedirs('model_x2_run2_data3/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_x2_run2/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_x2_run2_data3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_x2_run2_data3/dist_%s.mat'%(ee)
