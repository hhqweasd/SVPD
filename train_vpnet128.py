...
from model_vp_conic_128 import VanishingNet
from datasetis128 import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="6,7,3,4,5,0,1,2"
if not os.path.exists('model_vpnet128'):
    os.mkdir('model_vpnet128')
log_path = os.path.join('model_vpnet128', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vpnet128/vpnet_%d.pth' % (epoch + 1)))
