...
os.environ["CUDA_VISIBLE_DEVICES"]="4,1,3,2,5,6,7,0"
...
for ee in range(100,75,-1):
    ee=46
    generator_1='model_abl_sft2_1_run3/vpnet_%s.pth'%(ee)
    generator_2='model_abl_sft2_1_run3/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_abl_sft2_1_run3/maskis'):
        os.makedirs('model_abl_sft2_1_run3/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_abl_sft2_1_run3/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_abl_sft2_1_run3/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_abl_sft2_1_run3/dist_%s.mat'%(ee)
