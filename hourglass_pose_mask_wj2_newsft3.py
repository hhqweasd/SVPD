...
class SFTLayer(nn.Module): 
     def __init__(self,inp,oup): 
         ...
         self.SFT_scale_conv1 = nn.Conv2d(oup, oup, 1)
         self.SFT_shift_conv0 = nn.Conv2d(inp, oup, 1) 
         self.SFT_shift_conv1 = nn.Conv2d(oup, oup, 1) 
         
     def forward(self, x, xmask): 
        # x[0]: fea; x[1]: cond
        # scale = self.SFT_scale_conv1(F.leaky_relu(self.SFT_scale_conv0(x[1]), 0.1, inplace=True))
        scale = self.SFT_scale_conv1(F.leaky_relu(self.SFT_scale_conv0(xmask), 0.1, inplace=True))
        # shift = self.SFT_shift_conv1(F.leaky_relu(self.SFT_shift_conv0(x[1]), 0.1, inplace=True))
        shift = self.SFT_shift_conv1(F.leaky_relu(self.SFT_shift_conv0(xmask), 0.1, inplace=True))
