...
from model_vp_conic_dia3_256 import VanishingNet
from datasets_vp_full_256 import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="0,5,4,7,2,3,6,1"
...
if not os.path.exists('model_vp_conic_ori_dia3_256'):
    os.mkdir('model_vp_conic_ori_dia3_256')
opt.log_path = os.path.join('model_vp_conic_ori_dia3_256', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_vp_conic_ori_dia3_256/vpnet_%d.pth' % (epoch + 1)))
