...
from datasets_vp4_ori import ImageDataset
import numpy as np
from skimage import io,color
from skimage.transform import resize

from hourglass_pose_6 import hg
from model_vp_conic import VanishingNet

os.environ["CUDA_VISIBLE_DEVICES"]="1,2,3,4,0,7,5,6"
...
if not os.path.exists('model_vp_conic_ori_6'):
    os.mkdir('model_vp_conic_ori_6')
opt.log_path = os.path.join('model_vp_conic_ori_6', str(datetime.datetime.now()) + '.txt')
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        result = vpnet(torch.cat((real_nsr,real_nsr*mask),1),realco)
    ...
    torch.save(vpnet.state_dict(), 'model_vp_conic_ori_6/vpnet.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(vpnet.state_dict(), ('model_vp_conic_ori_6/vpnet_%d.pth' % (epoch + 1)))
