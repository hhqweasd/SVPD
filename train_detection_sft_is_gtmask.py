...
if not os.path.exists('model_detection_sft_is_gtmask'):
    os.mkdir('model_detection_sft_is_gtmask')
log_path = os.path.join('model_detection_sft_is_gtmask', str(datetime.datetime.now()) + '.txt')
mode='train'
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch>89:
        torch.save(vpnet.state_dict(), ('model_detection_sft_is_gtmask/vpnet_%d.pth' % (epoch + 1)))
