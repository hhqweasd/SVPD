...
os.environ["CUDA_VISIBLE_DEVICES"]="5,1,4,3,2,0,6,7"
...
for ee in range(100,90,-1):
    # ee=92
    generator_1='model_abl_sft_run1/vpnet_%s.pth'%(ee)
    generator_2='model_abl_sft_run1/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_abl_sft_run1/maskis'):
        os.makedirs('model_abl_sft_run1/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_abl_sft_run1/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_abl_sft_run1/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_abl_sft_run1/dist_%s.mat'%(ee)
