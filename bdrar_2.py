...
class BDRAR(nn.Module):
    ...
    def forward(self, x):
        ...
        predict1_hl_vp = self.predict_vp(refine1_hl_1)
        predict4_lh_vp = self.predict_vp(refine4_lh_1)
        
        fuse_attention_vp = F.sigmoid(self.fuse_attention_vp(torch.cat((refine1_hl_1, refine4_lh_1), 1)))
        fuse_predict_vp = torch.sum(fuse_attention_vp * torch.cat((predict1_hl_vp, predict4_lh_vp), 1), 1, True)
        

        return fuse_predict, predict1_hl, predict2_hl, predict3_hl, predict4_hl, predict1_lh, predict2_lh, predict3_lh, predict4_lh,fuse_predict_vp
