...
for ee in range(100,90,-1):
    generator_1='model_vpnet_withmask/vpnet_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask_IS,realco) in enumerate(dataloader):
        ...
        with torch.no_grad():
            score = vpnet(torch.cat((real_nsr,mask_IS),1),vpts)[:, -1].cpu().numpy()
        ...
        for res in range(1, len(multires)):
            ...
            vps = np.vstack(vpts)
            with torch.no_grad():
                score = vpnet(torch.cat((real_nsr,mask_IS),1),vps)[:, -res - 1].cpu().numpy().reshape(1, -1)
    ...
    err = np.array(err)
    nnn='model_vpnet_withmask/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_vpnet_withmask/dist_%s.mat'%(ee)
