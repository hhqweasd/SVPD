...
from model_detection2_sft_test import VanishingNet
...
for ee in range(100,90,-1):
    generator_1='model_detection2_sft/vpnet_%s.pth'%(ee)
    generator_2='model_detection2_sft/sdnet_%s.pth'%(ee)
    generator_3='model_detection2_sft/sdnet_IS_%s.pth'%(ee)
    ...
    for i, (real_nsr,mask,realco,mask_IS) in enumerate(dataloader):
        ...
        save_result='./model_detection2_sft/mask.png'
    ...
    err = np.array(err)
    nnn='model_detection2_sft/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection2_sft/dist_%s.mat'%(ee)
