...
from datasets_vp3 import ImageDataset
import numpy as np
...
if not os.path.exists('model_vp3_b8'):
    os.mkdir('model_vp3_b8')
opt.log_path = os.path.join('model_vp3_b8', str(datetime.datetime.now()) + '.txt')
...
input_A = Tensor(opt.batchSize, 3, 512, 512)
input_B = Tensor(opt.batchSize,2)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_nsr,mask,realco) in enumerate(dataloader):
        ...
        '''
        slabimage480_realsr = real_nsr.data
        ...
        slabimage480_realsr=resize(slabimage480_realsr,(500,500,3))
        io.imsave('./model_vp3_b8/input.png',slabimage480_realsr)
        '''
    ...
    torch.save(netG_A2B.state_dict(), 'model_vp3_b8/netG_A2B.pth')

    if (epoch + 1) % opt.snapshot_epochs == 0:
        torch.save(netG_A2B.state_dict(), ('model_vp3_b8/netG_A2B_%d.pth' % (epoch + 1)))
