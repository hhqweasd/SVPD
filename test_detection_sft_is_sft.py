...
from model_detection_sft_is_sft_test import VanishingNet
# from datasets_test import ImageDataset
from datasetis_test import ImageDataset

os.environ["CUDA_VISIBLE_DEVICES"]="6,5,4,7,1,3,0,2"
...
for ee in range(100,90,-1):
    generator_1='model_detection_sft_is_sft_data3_run1/vpnet_%s.pth'%(ee)
    generator_2='model_detection_sft_is_sft_data3_run1/sdnet_%s.pth'%(ee)
    ...
    if not os.path.exists('model_detection_sft_is_sft_data3_run1/maskis'):
        os.makedirs('model_detection_sft_is_sft_data3_run1/maskis')
    ...
    for i, (real_nsr,realco) in enumerate(dataloader):
        ...
        # save_result=join_paths('./model_detection_sft_is_sft_data3_run1/maskis/%s'% (str(i) + '.png'))
    ...
    err = np.array(err)
    nnn='model_detection_sft_is_sft_data3_run1/err_%s.mat'%(ee)
    scipyio.savemat(nnn, {'name': err})
    errdist = np.array(errdist)
    nnn='model_detection_sft_is_sft_data3_run1/dist_%s.mat'%(ee)
